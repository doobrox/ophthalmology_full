<div>
    ...
    <div class="relative">
        <div class="flex justify-between items-center mb-2">
            <div class="h-10 flex items-center w-3/5">
            ...
            @if($this->activeFilters)
                <span class="ml-1 text-blue-400 text-center uppercase">FILTERS ACTIVE</span>
            @endif
        ...
        <div wire:loading.class="opacity-50" class="@unless($complex || $this->hidePagination) rounded-b-none @endunless max-w-screen overflow-x-scroll bg-white px-5 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            <div style="min-height: 571px;">
                <div class="table align-middle min-w-full">
                        ...
                        @foreach($this->columns as $index => $column)
                            ...
                            @elseif($column['type'] === 'checkbox')
                                @unless($column['hidden'])
                                    <div class=" table-cell h-12 w-32 py-1 flex justify-center overflow-hidden align-middle px-1 border-b border-gray-200 bg-white text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider focus:outline-none">
                    ...
                    <div class="table-row" style="background-color: rgba(240, 245, 255, var(--tw-bg-opacity));">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                {{--<div class="overflow-hidden align-top bg-blue-200 px-1 py-1 pt-1 border-gray-200 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none"> --}}
                                <div class="overflow-hidden align-top bg-blue-100 px-1 py-1 border-b border-gray-200 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                    {{--<div>SELECT ALL</div>--}}
                                    <div>
                            ...
                            @elseif($column['type'] === 'label')
                                <div class="table-cell overflow-hidden align-top border-b dark:border-dark-5">
                                    {{ $column['label'] ?? '' }}
                                </div>
                            @else
                                <div class="table-cell overflow-hidden align-top border-b dark:border-dark-5">
                    ...
                    @forelse($this->results as $row)
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}" onclick="{{ $this->rowAction($row) }}">
    ...
    font-weight: 400;" class="table-cell border-b dark:border-dark-5 whitespace-nowrap @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}" onclick="{{ $this->cellAction($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
                ...
                <div class="p-2 sm:flex items-center justify-between">
                    ...
                    @if(count($this->results))
                        <div class="my-2 sm:my-0 flex items-center">
                            <select name="perPage" class="border mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
